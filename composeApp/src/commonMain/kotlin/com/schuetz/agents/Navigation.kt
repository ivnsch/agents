...
data class ChatNav(val id: Long, val name: String, val isMe: Boolean)

object NavConversions {
    fun toChatNav(agent: AgentData) = ChatNav(agent.id, agent.name, agent.isMe)
    fun ChatNav.toAgentData() = AgentData(this.id, this.name, this.isMe)
}
